...
df_order_stats.display()

# COMMAND ----------

df_review=spark.table('`02_silver`.fact_reviews')
...
df_fav_restaurant.display()

# COMMAND ----------

window1=Window.partitionBy(col('customer_id')).orderBy(col('item_quantity').desc())
df_fact_order_items=spark.table('`02_silver`.fact_order_item')
...
df_customer=spark.table('`02_silver`.dim_customers')
